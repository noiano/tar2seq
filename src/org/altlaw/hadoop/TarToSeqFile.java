...
public class TarToSeqFile {
    ...
    private boolean activate_block_compression = false;
    
    public void activate_block_compression(boolean compression_switch) {
		activate_block_compression = compression_switch;
	}

	protected static final Logger logger = Logger.getLogger(TarToSeqFile.class);
    ...
    public void execute() throws Exception {
        ...
        try {
            input = openInputFile();
            output = openOutputFile(activate_block_compression);
            TarEntry entry;
            while ((entry = input.getNextEntry()) != null) {
                ...
                logger.info("Processing file "+entry.getName() +" size:"+entry.getSize());
    ...
    private SequenceFile.Writer openOutputFile(boolean enable_compression) throws Exception {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (args[0].compareTo("-c") == 0){
            	logger.info("Turning on compression");
                me.setInput(new File(args[1]));
                me.setOutput(new File(args[2]));
                me.activate_block_compression(true);
            }
            else
            {
            	logger.info("Arg[0] is "+args[0]);
            	logger.info("No compression will be used");
            	me.setInput(new File(args[0]));
                me.setOutput(new File(args[1]));
                me.activate_block_compression(false);
            }
    ...
    public static void exitWithHelp() {
        System.err.println("Usage: java org.altlaw.hadoop.TarToSeqFile [-c] <tarfile> <output>\n\n" +
                           "<tarfile> may be GZIP or BZIP2 compressed, must have a\n" +
                           "recognizable extension .tar, .tar.gz, .tgz, .tar.bz2, or .tbz2."+
                           "\nUse -c option to enable record compression");
        System.exit(1);
    }
}
